...
class VanillaTemporalModule(nn.Module):
    ...
    def forward(self, input_tensor, temb, encoder_hidden_states, attention_mask=None, vision_encoder_hidden_states=None,
                anchor_frame_idx=None):
        hidden_states = input_tensor
        hidden_states = self.temporal_transformer(hidden_states, encoder_hidden_states, vision_encoder_hidden_states,
                                                  attention_mask)
...
class TemporalTransformer3DModel(nn.Module):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, vision_encoder_hidden_states=None,
                attention_mask=None):
        ...
        for block in self.transformer_blocks:
            hidden_states = block(hidden_states, encoder_hidden_states=encoder_hidden_states, \
                                  vision_encoder_hidden_states=vision_encoder_hidden_states, video_length=video_length,
                                  shape=shape)
        ...
        for block_name in attention_block_types:
            if 'Self' in block_name:
                ...
                attention_block = VersatileCrossAttention(
                    ...
                    vision_cross_attention_dim=vision_cross_attention_dim if block_name.endswith(
                        "_Vision_Cross") else None,
...
class VersatileAttention(CrossAttention):
    ...
    def extra_repr(self):
        ...
        # TODO added from old version of diffusers - should figure how to fast forward this
...
class VersatileSelfAttention(CrossAttention):
    def __init__(
        ...
        self._use_memory_efficient_attention_xformers = False  # TODO just set to this for now
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, vision_encoder_hidden_states=None, attention_mask=None,
        ...
        if self._use_memory_efficient_attention_xformers:
            ...
            self._slice_size = None # TODO NOT SURE ABOUT THIS
            if self._slice_size is None or query.shape[0] // self._slice_size == 1:
                hidden_states = self._attention(query, key, value, attention_mask)
            else:
                hidden_states = self._sliced_attention(query, key, value, sequence_length, dim, attention_mask)
...
class VersatileCrossAttention(CrossAttention):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, vision_encoder_hidden_states=None, attention_mask=None,
        ...
        encoder_hidden_states = repeat(encoder_hidden_states, "b n c -> (b d) n c",
                                       d=sequence_length) if self.is_text_cross_attention else None
        vision_encoder_hidden_states = repeat(vision_encoder_hidden_states, "b n c -> (b d) n c",
                                              d=sequence_length) if self.is_vision_cross_attention else None
...
class LightAttention(CrossAttention):
    ...
    #     tensor = tensor.reshape(batch_size, seq_len, head_size, dim // head_size).transpose(0, 2, 1, 3)
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, vision_encoder_hidden_states=None, attention_mask=None,
        ...
        if encoder_hidden_states.shape[-2] > 12 and self.kwargs['down_resize'] == 'True':
            ...
            encoder_hidden_states = torch.nn.functional.interpolate(encoder_hidden_states, (w_, h_)).to(
